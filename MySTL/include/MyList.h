...
namespace myStd
{
	...
	{
	public:
		MyList() : sizeValue(0), head(nullptr), tail(nullptr) {};
		...
		const MyList<T>& operator=(const MyList<T>& other);
		const MyList<T>& operator=(MyList<T>&& other);
	...
	template<typename T>
	MyList<T>::MyList(std::initializer_list<T> initList) : sizeValue(initList.size())
	{
		head = new Node<T>;
		Node<T>* current = head;
		Node<T>* next = nullptr;
	...
	template<typename T>
	MyList<T>::MyList(const MyList& other) : sizeValue(other.sizeValue)
	{
		head = new Node<T>;
		Node<T>* next = nullptr;
		Node<T>* current = head;
	...
	template<typename T>
	const MyList<T>& MyList<T>::operator=(MyList<T>&& other)
	{
		if (this != &other) {
			clear();
			sizeValue = other.sizeValue;
	...
	template<typename T>
	void MyList<T>::insert(const int index, T value)
	{
	...
	template<typename T>
	void MyList<T>::erase(const int index)
	{
	...
	template<typename T>
	void MyList<T>::assign(int size, T value)
	{
		clear();
		sizeValue = size;
		head = new Node<T>;
