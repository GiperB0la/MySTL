...
namespace myStd
{
	...
	public:
		...
		MyDeque(std::initializer_list<T> initList) : sizeValue(initList.size()) {
			int i = 0;
		...
		const MyDeque<T>& operator=(const MyDeque<T>& other);
		const MyDeque<T>& operator=(MyDeque<T>&& other);
	...
	template<typename T>
	const MyDeque<T>& MyDeque<T>::operator=(const MyDeque<T>& other)
	{
		if (this != &other) {
			blocks.clear();
			blocks = other.blocks;
	...
	template<typename T>
	const MyDeque<T>& MyDeque<T>::operator=(MyDeque<T>&& other)
	{
		if (this != &other) {
			blocks.clear();
			blocks = other.blocks;
