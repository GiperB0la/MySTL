...
namespace myStd
{
	...
	{
	public:
		MyUnordered_map() : bucketValue(2), sizeValue(0) {
	...
	template<typename T1, typename T2>
	void MyUnordered_map<T1, T2>::inBucket(bool flag)
	{
		if (flag)
			bucketValue *= 2;
	...
	template<typename T1, typename T2>
	void MyUnordered_map<T1, T2>::insert(T1 key, T2 meaning)
	{
		if (!find(key)) {
	...
	template<typename T1, typename T2>
	bool MyUnordered_map<T1, T2>::find(T1 key)
	{
	...
	template<typename T1, typename T2>
	void MyUnordered_map<T1, T2>::erase(T1 key)
	{
		if (find(key)) {
			for (myStd::MyVector<myStd::string> n : bucket[hash(key)]) {
				for (myStd::string h : n) {
	...
	template<typename T1, typename T2>
	int MyUnordered_map<T1, T2>::get(T1 key)
	{
		if (find(key)) {
			for (myStd::MyVector<myStd::string> n : bucket[hash(key)]) {
				for (myStd::string h : n) {
