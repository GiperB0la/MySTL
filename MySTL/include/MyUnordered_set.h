...
namespace myStd
{
	...
	{
	public:
		MyUnordered_set() : bucketValue(2), sizeValue(0) {
	...
	template<typename T>
	void MyUnordered_set<T>::inBucket(bool flag)
	{
		if (flag)
			bucketValue *= 2;
	...
	template<typename T>
	void MyUnordered_set<T>::add(T value)
	{
		if (!contains(value)) {
	...
	template<typename T>
	int MyUnordered_set<T>::contains(T value)
	{
	...
	template<typename T>
	void MyUnordered_set<T>::remove(T value)
	{
		if (contains(value)) {
			for (auto n : bucket[hash(value)]) {
				if (n == value) {
