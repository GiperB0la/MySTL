...
namespace myStd
{
	...
	{
	public:
		MyForward_list() : sizeValue(0), head(nullptr), tail(nullptr) {};
		MyForward_list(std::initializer_list<T> initList);
		MyForward_list(const MyForward_list& other);
		...
		const MyForward_list<T>& operator=(const MyForward_list<T>& other);
		const MyForward_list<T>& operator=(MyForward_list<T>&& other);
	...
	template<typename T>
	MyForward_list<T>::MyForward_list(std::initializer_list<T> initList) : sizeValue(initList.size())
	{
		head = new Node<T>;
		Node<T>* current = head;
		Node<T>* next = nullptr;
	...
	template<typename T>
	MyForward_list<T>::MyForward_list(const MyForward_list& other) : sizeValue(other.sizeValue)
	{
		head = new Node<T>;
		Node<T>* current = head;
		Node<T>* last = nullptr;
	...
	template<typename T>
	T& MyForward_list<T>::operator[](const int index)
	{
	...
	template<typename T>
	const MyForward_list<T>& MyForward_list<T>::operator=(const MyForward_list<T>& other)
	{
		if (this != &other) {
	...
	template<typename T>
	const MyForward_list<T>& MyForward_list<T>::operator=(MyForward_list<T>&& other)
	{
		if (this != &other) {
			sizeValue = other.sizeValue;
			head = other.head;
	...
	template<typename T>
	T& MyForward_list<T>::at(int index)
	{
		if (index >= 0 && index < sizeValue) {
			int n = 0;
			Node<T>* current = head;
	...
	template<typename T>
	void MyForward_list<T>::push_back(T value)
	{
		if (head == nullptr) {
	...
	template<typename T>
	void MyForward_list<T>::push_front(T value)
	{
		if (head == nullptr) {
			head = new Node<T>(value);
		}
	...
	template<typename T>
	void MyForward_list<T>::pop_back()
	{
		Node<T>* current = head;
		Node<T>* temp = current;
		while (current != tail) {
	...
	template<typename T>
	void MyForward_list<T>::pop_front()
	{
		Node<T>* temp = head;
		head = head->pNext;
		delete temp;
	...
	template<typename T>
	void MyForward_list<T>::insert(const int index, T value)
	{
	...
	template<typename T>
	void MyForward_list<T>::erase(int index)
	{
		if (head == nullptr) return;
