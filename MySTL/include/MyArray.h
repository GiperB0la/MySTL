...
namespace myStd
{
	...
	{
	public:
		MyArray() : sizeValue(S) { mArray = new T[sizeValue]; }
		MyArray(const MyArray& other);
		MyArray(MyArray&& other);
		MyArray(std::initializer_list<T> initList);
		~MyArray() { delete[] mArray; }

		const MyArray<T, S>& operator=(const MyArray<T, S>& other);
		const MyArray<T, S>& operator=(MyArray<T, S>&& other) noexcept;
	...
	template<typename T, int S>
	MyArray<T, S>::MyArray(const MyArray& other) : sizeValue(other.sizeValue)
	{
	...
	template<typename T, int S>
	MyArray<T, S>::MyArray(MyArray&& other) : sizeValue(other.sizeValue)
	{
		mArray = other.mArray;
	...
	template<typename T, int S>
	MyArray<T, S>::MyArray(std::initializer_list<T> initList) : sizeValue(initList.size())
	{
		mArray = new T[sizeValue];
		int i = 0;
	...
	template<typename T, int S>
	const MyArray<T, S>& MyArray<T, S>::operator=(const MyArray<T, S>& other)
	{
		if (this != &other) {
			delete[] mArray;
	...
	template<typename T, int S>
	const MyArray<T, S>& MyArray<T, S>::operator=(MyArray<T, S>&& other) noexcept
	{
		if (this != &other) {
			delete[] mArray;
			sizeValue = other.sizeValue;
	...
	template<typename T, int S>
	T& MyArray<T, S>::operator[](const int index)
	{
